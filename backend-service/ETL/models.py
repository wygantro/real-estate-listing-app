...
class User(UserMixin, db.Model):
    ...
    # account settings
    email_weekly = db.Column(db.Boolean, default=False,
                             server_default="false",index=True)
    user_setting_option1 = db.Column(db.Boolean, default=False,
                                     server_default="false", index=True)
    user_setting_option2 = db.Column(db.Boolean, default=False,
                                     server_default="false", index=True)
    
    # relationships
    user_listings = db.relationship('User_listing', backref='user', lazy='dynamic',
                                    cascade='all, delete, delete-orphan')
    user_counties = db.relationship('User_county', backref='user', lazy='dynamic',
                                    cascade='all, delete, delete-orphan')
...
class Listing(db.Model):
    ...
    redemption_period = db.Column(db.String(150), index=False, unique=False)
    mortgagor_name = db.Column(db.String(150), index=False, unique=False)
    foreclosing_assignee_mortgagee = db.Column(db.String(150),
    ...
    listing_active = db.Column(db.Boolean, default=False,
                               server_default="false", index=True)
    date_relisted_uploaded = db.Column(db.DateTime(),
    ...
    # relationships
    user_listings = db.relationship(
        'User_listing',
        backref='listing',
        lazy='dynamic',
        cascade='all, delete, delete-orphan')
    image_files = db.relationship(
        'Image_files',
        backref='listing',
        lazy='dynamic',
        cascade='all, delete, delete-orphan')
...
class County(db.Model):
    ...
    LISTINGS_AVAILABLE = db.Column(db.Boolean, default=False,
                                   server_default="false", index=True)
    ...
    # relationships
    listings = db.relationship('Listing', backref='county', lazy='dynamic', 
                               cascade='all, delete, delete-orphan')
    user_counties = db.relationship('User_county', backref='county', lazy='dynamic',
                                    cascade='all, delete, delete-orphan')
...
# user county access tabel
class User_county(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), index=True)
    county_id = db.Column(db.Integer, db.ForeignKey('county.id'), index=True)
    monthly_subscribed = db.Column(db.Boolean, default=False,
                                   server_default="false", index=True)
    yearly_subscribed = db.Column(db.Boolean, default=False,
                                  server_default="false",index=True)
    payment_agreement = db.Column(db.Boolean, default=False,
                                  server_default="false", index=True)
    subscribed_at_date = db.Column(db.DateTime(), index=True,
                                   default=datetime.utcnow)


# image file info tabel
